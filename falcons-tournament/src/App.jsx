...
const makeDay2Games = (teams, sched, pfx) =>
  sched.map(([i,j,time], k) => ({ id:`${pfx}${k}`, time, team1:teams[i], team2:teams[j], s1:"", s2:"", soResult:null }));
...
const makePlayoffGames = (seeds, pfx, times) => {
  ...
  return [
    { id:`${pfx}sf1`, label:"Semi-Final 1", time:t1, team1:s1, team2:s4, s1:"", s2:"", ot:false, soResult:null },
    { id:`${pfx}sf2`, label:"Semi-Final 2", time:t2, team1:s2, team2:s3, s1:"", s2:"", ot:false, soResult:null },
    { id:`${pfx}b`,   label:"Bronze Medal", time:tb, team1:null, team2:null, s1:"", s2:"", ot:false, soResult:null },
    { id:`${pfx}g`,   label:"Gold Medal",   time:tg, team1:null, team2:null, s1:"", s2:"", ot:false, soResult:null },
  ];
};
...
function refreshFinals(games) {
  ...
  if (played(g[0]) && played(g[1])) {
    ...
    g[2] = { ...g[2], team1:null, team2:null, s1:"", s2:"", soResult:null };
    g[3] = { ...g[3], team1:null, team2:null, s1:"", s2:"", soResult:null };
  }
  return g;
}
...
// ─── Standings Table ───────────────────────────────────────────
function StandingsTable({ teams, games, cutAt, topTag, botTag }) {
  const rows = calcStandings(teams, games);
  return (
      ...
      <table className="w-full" style={{ fontSize:11, borderCollapse:"collapse" }}>
        <thead>
          <tr style={{ background:"rgba(0,0,0,0.3)", color:"#4a6a9c" }}>
            ...
            {["GP","W","OW","T","OL","L","GF","GA"].map(h => (
              <th key={h} className="px-1 py-1.5 text-center"
                style={h==="T" ? {color:"#fbbf24"} : {}}>{h}</th>
            ))}
          ...
          {rows.map((s, i) => {
            ...
            return (
              <tr key={s.team} style={{
                ...
                {[s.gp,s.w,s.otw,s.t,s.otl,s.l,s.gf,s.ga].map((v, j) => (
                  <td key={j} className="px-1 py-1.5 text-center"
                    style={{ color: j===3 && v>0 ? "#fbbf24" : "#8aa0c8" }}>{v}</td>
                ))}
...
// ─── Group Panel ──────────────────────────────────────────────
function GroupPanel({ title, accent, teams, games, onGamesChange, cutAt, topTag, botTag, locked, reorderable }) {
...
export default function HockeyTournament() {
  ...
  const buildDay2GamesFromOrder = (teams, sched, order, pfx) => {
    ...
    return order.map((schedIdx, slotIdx) => {
      ...
      return { id:`${pfx}${schedIdx}`, time:times[slotIdx], team1:teams[i], team2:teams[j], s1:"", s2:"", soResult:null };
    });
  };
  ...
  const goDay3 = () => {
    const [sA, sB] = [calcStandings(gA.teams, gA.games), calcStandings(gB.teams, gB.games)];
    ...
    <div style={{ minHeight:"100vh", background:"#070f1c", fontFamily:"'DM Sans', sans-serif", color:"#c8d8f0" }}>
      ...
      <div style={{ maxWidth:1100, margin:"0 auto", padding:"24px 16px" }}>
                ...
                {isProjection ? (
                  <>
                    <GroupPanel title="Saturday · Group B (projected)" accent="linear-gradient(135deg,#7f1d1d,#b91c1c)"
                      ...
                      locked={true} reorderable={!locked} />
        ...
        {phase === "day3" && (() => {
          ...
          const sA = calcStandings(projA_teams, projA_games);
          const sB = calcStandings(projB_teams, projB_games);
